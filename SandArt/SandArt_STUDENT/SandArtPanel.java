...
{	
   ...
   private static int weightIndex;                 //weight of snow - determines how they settle
                                                   //x and y mins and max for color selection sliders
   private static int colorXmin, colorXmax, rYmin, rYmax, gYmin, gYmax, bYmin, bYmax;
   ...
   public SandArtPanel()
   {
      ...
      colorXmin  = (screenWidth +  textSize/2)+(textSize*3);
      colorXmax  = (screenWidth +  textSize/2)+(textSize*3)+100;
      int yStart = /*(textSize/2) + */(textSize*22);
      rYmin = yStart;
      rYmax = rYmin + textSize;
      gYmin = yStart + (textSize);
      gYmax = gYmin + textSize;
      bYmin = yStart + (textSize * 2);
      bYmax = bYmin + textSize;
 ...
 //post:  removes all plaforms (White colored particles)
      ...
      for(int r = 0; r < snow.length; r++)
      {
         for(int c = 0; c < snow[0].length; c++)
         {
            ...
            if(snowColor != null && snowColor.equals(Color.white))
               snow[r][c] = new Color(254, 254, 254);
         }
      }
   }
  ...
  //post:  display all snow, platforms and instructions on the screen
   public static void showBoard(Graphics g)	
   {
      ...
      g.fillRect(x+(textSize*3)+110, y+textSize/2, textSize, textSize*2);
      g.setColor(Color.lightGray);
      //slider bars for color selection
      g.drawLine(colorXmin, rYmin, colorXmax, rYmin); 
      g.drawLine(colorXmin, rYmin+textSize, colorXmax, rYmin+textSize); 
      g.drawLine(colorXmin, rYmin+textSize*2, colorXmax, rYmin+textSize*2); 
      //draw dot to show color position on slider bar for each color
      g.setColor(Color.red);
      int redX = (int)((currentColor.getRed()/255.0) * 100) + colorXmin;
      g.fillOval(redX, rYmin - (textSize/2), textSize, textSize);
      
      g.setColor(Color.green);
      int greenX = (int)((currentColor.getGreen()/255.0) * 100) + colorXmin;
      g.fillOval(greenX, gYmin - (textSize/2), textSize, textSize);
   
      g.setColor(Color.blue);
      int blueX = (int)((currentColor.getBlue()/255.0) * 100) + colorXmin;
      g.fillOval(blueX, bYmin - (textSize/2), textSize, textSize);
   
      g.setColor(Color.lightGray);
      g.drawString("r/R:", x, y+=textSize);
      g.drawString("g/G:", x, y+=textSize);
      g.drawString("b/B:", x, y+=textSize);
...
//post:  performs action depending on what key the user hit
   public void processUserInput(int k, boolean shiftPressed)
   {
      ...
      if(k==KeyEvent.VK_R || k==KeyEvent.VK_G || k==KeyEvent.VK_B)
      {
...
//post: given the position of the mouse, returns RED, GREEN or BLUE if it is in a color slider area, -1 if not
   public static int mouseSliderLocation(int mouseX, int mouseY)
   {
      if(mouseX>=colorXmin && mouseX<=colorXmax && mouseY>=rYmin && mouseY<=rYmax)
      {
         return RED;  
      }
      else if(mouseX>=colorXmin && mouseX<=colorXmax && mouseY>=gYmin && mouseY<=gYmax)
      {
         return GREEN;  
      }
      else if(mouseX>=colorXmin && mouseX<=colorXmax && mouseY>=bYmin && mouseY<=bYmax)
      {
         return BLUE;  
      }
      return -1;
 ...
 //***BEGIN MOUSE STUFF***
   ...
   public void mouseClicked( MouseEvent e )
   {
      ...
      //see if mouse is being clicked in a slider bar for colors: Red, Green, Blue
      int red = currentColor.getRed();
      int green = currentColor.getGreen();
      int blue = currentColor.getBlue();
   
      int sliderColor = mouseSliderLocation(mouseX, mouseY);
   
      if(sliderColor == RED)
      {
         int currX = mouseX - colorXmin;
         red = (int)(255 * (currX/100.0));
         currentColor = new Color(red%256, green, blue);  
      }
      else if(sliderColor == GREEN)
      {
         int currX = mouseX - colorXmin;
         green = (int)(255 * (currX/100.0));
         currentColor = new Color(red, green%256, blue);  
      }
      else if(sliderColor == BLUE)
      {
         int currX = mouseX - colorXmin;
         blue = (int)(255 * (currX/100.0));
         currentColor = new Color(red, green, blue%256);  
      }
      repaint();
   }
   ...
   public void mouseDragged( MouseEvent e)
   {
      ...
      int sliderColor = mouseSliderLocation(mouseX, mouseY);
      if(sliderColor != -1)                                    //mouse is being dragged in R,G or B slider area
      {
         if(sliderColor == RED)
         {
            int currX = mouseX - colorXmin;
      ...
      else if(cursorMode==DRAW && isValid(mouseY, mouseX))     //paint tool to make platforms
      {
